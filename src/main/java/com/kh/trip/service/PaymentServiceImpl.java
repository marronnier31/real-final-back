	...
	public Long save(PaymentDTO paymentDTO, Long userNo) {
		...
		if (usedMileage > 0) {
			...
			if (!alreadyUsed) {
				...
				MileageHistory mileageHistory = MileageHistory.builder().user(user).booking(booking).payment(result)
						.changeType(MileageChangeType.USE).changeAmount(usedMileage).balanceAfter(user.getMileage())
						.reason("예약 결제 시 마일리지 사용").status(MileageStatus.NORMAL).build();
	...
	public void cancel(Long paymentNo, Long userNo) {
		...
		for (MileageHistory history : histories) {
			...
			if (history.getChangeType() == MileageChangeType.USE && history.getStatus() == MileageStatus.NORMAL) {
				...
				MileageHistory restoreHistory = MileageHistory.builder().user(user).booking(history.getBooking())
						.payment(payment).changeType(MileageChangeType.CANCEL_USE)
						.changeAmount(history.getChangeAmount()).balanceAfter(user.getMileage())
						.reason("결제 취소로 사용 마일리지 복구").status(MileageStatus.NORMAL).build();
